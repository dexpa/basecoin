...
(./basecli-reserve tx send --name=${RESERVE} --amount="1000CLR x" --to=${TEST1_ADDR} --sequence=$((SEQR+1))) < passphrase.dat 
(./basecli-reserve tx send --name=${RESERVE} --amount="1000CLR x" --to=${TEST2_ADDR} --sequence=$((SEQR+2))) < passphrase.dat 
...
echo "Reserve ${AMT0}, Test1 ${AMT1} ${TAG1}, Test2 ${AMT2} ${TAG2}"
...
(./basecli tx send --name=${TEST1} --amount="10CLR 58e45a" --to=${RESER_ADDR} --sequence=$((SEQT1+2))) < passphrase.dat 
(./basecli tx send --name=${TEST2} --amount="10CLR 7d33ac" --to=${RESER_ADDR} --sequence=$((SEQT2+2))) < passphrase.dat 
